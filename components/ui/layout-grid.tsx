...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    <div className="w-full min-h-[600px] p-10 grid grid-cols-1 md:grid-cols-3 max-w-7xl mx-auto gap-4 relative">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "min-h-[300px]")}>
          <motion.div
            ...
            className={cn(
              ...
              selected?.id === card.id
                ? "rounded-lg cursor-pointer absolute inset-0 h-[80vh] w-full md:w-3/4 lg:w-2/3 m-auto z-50 flex justify-center items-center flex-wrap flex-col"
                : lastSelected?.id === card.id
                  ? "z-40 bg-gray-500 rounded-xl h-full w-full min-h-[300px]"
